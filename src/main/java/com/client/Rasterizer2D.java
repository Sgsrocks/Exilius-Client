...
public class Rasterizer2D extends Cacheable implements RSRasterizer2D {
    ...
    public static void draw_arc(int x, int y, int width, int height, int stroke, int start, int sweep, int color, int alpha, int closure, boolean fill) {
        Graphics2D graphics = Sprite.createGraphics(Rasterizer2D.pixels, Rasterizer2D.width, Rasterizer2D.height);
    ...
    public static void renderGlow(int drawX, int drawY, int glowColor, int r) {
        ...
        float edge0 = -(r / 2f);
        float edge1 = map((float) Math.sin(Client.update_tick / 20f), -1, 1, edge0 + (r / 1.35f), r);
